...
int extractBlobs(cv::Mat fgmask, std::vector<cvBlob> &bloblist, int connectivity)
{
	...
	//Scalar newVal = 255; DONT FORGET TO ERASE
	...
	for (int i = 0; i < fgmask.rows; i++) {
		for (int j = 0; j < fgmask.cols; j++) {
			if (fgmask_copy.at<uchar>(i,j) == 255) {
				counter++;
				cv::floodFill(fgmask_copy, cv::Point(j,i), 32, &rect ,0,0, connectivity);
...
int classifyBlobs(std::vector<cvBlob> &bloblist)
{
	...
	for (int i = 0; i < bloblist.size(); i++) {
		...
		//...
		float ratio = float(blob.w)/float(blob.h);
		float dist_person = WED(ratio,MEAN_PERSON,STD_PERSON);
		float dist_car = WED(ratio,MEAN_CAR,STD_CAR);
		float dist_object = WED(ratio,MEAN_OBJECT,STD_OBJECT);

		if ((ratio >= MEAN_PERSON - STD_PERSON) && (ratio <= MEAN_PERSON + STD_PERSON) && (dist_person < min(dist_car,dist_object))) {
			blob.label = PERSON;
		}
		else if ((ratio >= MEAN_CAR - STD_CAR) && (ratio <= MEAN_CAR + STD_CAR) && (dist_car < min(dist_person,dist_object))) {
			blob.label = CAR;
		}
		else if ((ratio >= MEAN_OBJECT - STD_OBJECT) && (ratio <= MEAN_OBJECT + STD_OBJECT) && (dist_object < min(dist_person,dist_car))) {
			blob.label = OBJECT;
		}
		else {
			blob.label = UNKNOWN;
...
#define SECS_STATIONARY 3 // 1 5
#define I_COST 3 // 3 1 // increment cost for stationarity detection
#define D_COST 10// 2 4// decrement cost for stationarity detection
	...
	for (int i = 0; i<fgmask.rows; i++) {
		...
		// increase FHI if it belongs to the foreground
			if (fgmask.at<uchar>(i,j)==255)
				fgmask_history.at<float>(i,j) += I_COST;
			else
				fgmask_history.at<float>(i,j) -= D_COST;
			 // void implementation (no history)
			fgmask_history.at<float>(i,j) = max(float(0), fgmask_history.at<float>(i,j));

		// update sfgmask
			normalized_fgmask_history.at<float>(i,j) = min(float(1), fgmask_history.at<float>(i,j)/(numframes4static));
